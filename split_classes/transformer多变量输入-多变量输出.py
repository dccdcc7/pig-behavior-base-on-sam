...
class Transformer(nn.Module):
    ...
    def __init__(self, n_encoder_inputs, n_decoder_inputs, Sequence_length, d_model=512, dropout=0.1, num_layer=8):
        ...
        self.ziji_add_linear = torch.nn.Linear(Sequence_length, 6)
...
model = Transformer(n_encoder_inputs=6, n_decoder_inputs=6, Sequence_length=9).to(device)  # 3 表示Sequence_length  transformer 输入数据 序列的长度
...
def _test():
    with torch.no_grad():
        ...
        for index, (inputs, targets) in enumerate(TrainDataLoader):
            ...
            tgt_in = torch.rand((Batch_Size,9,6))
...
for epoch in tqdm(range(epochs)):
    ...
    for index, (inputs, targets) in enumerate(TrainDataLoader):
        ...
        tgt_in = torch.rand((Batch_Size,9,6))  # 输入数据的维度是[batch,序列长度，每个单元的维度]
        outputs = model(inputs, tgt_in)
...
# 加载模型预测------
model = Transformer(n_encoder_inputs=6, n_decoder_inputs=6, Sequence_length=9).to(device)
