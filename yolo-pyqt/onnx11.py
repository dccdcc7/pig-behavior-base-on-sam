...
#在这个基础上进行qt的编写显然会更好，因为每次检验不用重新部署了，这样就非常方便，给人感觉也比较好
...
def draw(image, box_data):
    ...
    for box, score, cl in zip(boxes, scores, classes):
        ...
        num+=1
    num1=str(num)
    cv2.putText(image, num1, (20, 20), cv2.FONT_HERSHEY_SIMPLEX,0.6, (0, 0, 255), 2)
    return image,num
...
def onnx_forward():
    ...
    for entry in entries:
        entry = path+entry
        output, or_img = model.inference(entry)
        outbox = filter_box(output, 0.1, 0.4)  # 最终剩下的Anchors：0 1 2 3 4 5 分别是 x1 y1 x2 y2 score class
        print(outbox)
        import torch
        outbox1 = torch.tensor(outbox)
        outbox1 = outbox1.to('cuda')
        print(outbox1)
        # print('outbox( x1 y1 x2 y2 score class):')
        # print(outbox)
        if len(outbox) == 0:
            print('没有发现物体')
        or_img,num = draw(or_img, outbox)
        cv2.imshow('23',or_img)
        cv2.waitKey(0)
        list_num.append(num)
    return list_num,outbox1
    # cv2.imwrite('./res.jpg', or_img)


def model_init():
    onnx_path = 'pigbest.onnx'
    model = Yolov5ONNX(onnx_path)
    return model

def onnx_forward1(model,img):
    list_num = []
    #model = Yolov5ONNX(onnx_path)
    # output, or_img = model.inference('data/images/img.png')
    output, or_img = model.inference1(img)
    outbox = filter_box(output, 0.01, 0.4)  # 最终剩下的Anchors：0 1 2 3 4 5 分别是 x1 y1 x2 y2 score class
    #print(outbox)
    outbox1 = torch.tensor(outbox)
    outbox1 = outbox1.to('cuda')
    #print(outbox1)
...
# def video_forward():
#     onnx_path = 'runs/exp6/weights/best.onnx'
#     model = Yolov5ONNX(onnx_path)
#
#     # output, or_img = model.inference('data/images/img.png')
#     def get_file_path():
...
#         ret, frame = cap.read()
#         # 如果正确读取帧，ret为True
#         output, or_img = model.inference1(frame)
