import time, torch, torchvision, cv2, random
import numpy as np
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        t_size = cv2.getTextSize(label, 0, fontScale=tl*1/4 , thickness=tf*2)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, [0,0,255], -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl*1/4 , [255, 255, 255], thickness=tf*2, lineType=cv2.LINE_AA)
        # masks_all = []
        # # print(os.path.basename(t)[:-4])
        # predictor.set_image(img)
        # x = np.array(x)
        # masks, iou_predictions, low_res_masks = predictor.predict(box=x[:4])
        ...
        #     masks1 = 255 - masks1
        # masks = masks.astype(np.int32) * np.array(colors(x[4]))[:, None, None]
